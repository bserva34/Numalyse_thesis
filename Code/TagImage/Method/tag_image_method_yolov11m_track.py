...
def main():
    ...
    parser.add_argument('--thr', type=float, default=0.5, help='Detection score threshold')
    ...
    for i in tqdm(range(num_frames), desc="Histograms"):
        ...
        if len(batch_imgs) == batch_size or i == num_frames - 1:
            ...
            outputs = model.track(
                batch_imgs,
                device=device,
                persist=True,        # Indispensable pour garder la mémoire du batch/frame précédent
                tracker="bytetrack.yaml", # Optionnel mais recommandé (ByteTrack par défaut)
                verbose=False
            )
