...
#     for _ in tqdm(range(total_frames), desc="Reading and computing features", unit="frame"):
#         ret, frame = cap.read()
#         if not ret:
#             break

#         # 1. Calcul de l'histogramme HSV
#         hsv = cv2.cvtColor(frame, cv2.COLOR_BGR2HSV)
#         hist = cv2.calcHist([hsv], [0, 1, 2], None, [8, 3, 3], [0, 180, 0, 256, 0, 256])
#         hist = cv2.normalize(hist, None).flatten()

#         # 2. Calcul du flux optique et de la différence d'histogramme avec la frame précédente
#         if prev_frame is not None:
#             pd = np.linalg.norm(prev_hist - hist, ord=1)
...
#         prev_hist = hist
#         prev_frame = frame  # Conservé uniquement pour la comparaison avec la frame suivante

#     cap.release()

#     PD, GD = np.array(PD), np.array(GD)

#     # 3. Agrégation par fenêtre glissante
#     X, idxs = [], []
#     n_pairs = len(PD)
#     for i in tqdm(range(n_pairs - window + 1), desc="Aggregating features", unit="win"):
#         win_pd = PD[i: i + window]
...
# def save_selected_keyframes(video_path, target_indices, out_dir):
#     """Lit la vidéo à nouveau pour n'extraire et sauvegarder que les frames sélectionnées."""
#     cap = cv2.VideoCapture(video_path)
#     base = os.path.splitext(os.path.basename(video_path))[0]
    
#     # Association des indices de frames ciblés avec leurs positions dans la vidéo
#     indices_set = set(target_indices)
#     current_idx = 0
    
#     pbar = tqdm(total=len(indices_set), desc="Saving keyframes", unit="kf")
#     while cap.isOpened() and len(indices_set) > 0:
#         ret, frame = cap.read()
#         if not ret:
#             break
            
#         if current_idx in indices_set:
#             out_name = f"{base}_keyframe_{current_idx:06d}.jpg"
#             out_path = os.path.join(out_dir, out_name)
#             cv2.imwrite(out_path, frame)
#             indices_set.remove(current_idx)
#             pbar.update(1)
...
#     # 1. Extraction des features (sans stocker les frames)
#     X, idxs = extract_features(args.video, args.window)

#     # 2. Optimisation des hyperparamètres
#     nu_opt, gamma_opt = ispmde_optimize(X, args.kfold, args.pop, args.gens, args.F, args.cr)
#     print(f"Optimized nu={nu_opt:.4f}, gamma={gamma_opt:.4g}")

#     # 3. Entraînement final et scoring
#     scaler = MinMaxScaler().fit(X)
#     X_s = scaler.transform(X)
#     model = OneClassSVM(kernel='rbf', nu=nu_opt, gamma=gamma_opt)
#     model.fit(X_s)
...
#     # Récupération des indices réels de frames cibles
#     target_frame_indices = [idxs[sel] for sel in selected]

#     # 4. Sauvegarde ciblée des keyframes en réouvrant la vidéo
#     save_selected_keyframes(args.video, target_frame_indices, args.out)
#     print(f"Saved {len(selected)} keyframe(s) to {args.out}")
