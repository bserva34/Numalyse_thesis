...
def main():
    ...
    bin_edges = np.linspace(0, 100, 91 + 1)
    bin_centers = (bin_edges[:-1] + bin_edges[1:]) / 2

    plt.figure(figsize=(10, 6))

    # Option A : Tracé sous forme de barres (le plus classique)
    plt.bar(
        bin_centers, 
        hist_t, 
        ...
        color='skyblue', 
        alpha=0.8,
        label='Histogramme moyen pondéré'
    )

    # Configuration des axes et du style
    plt.title("Visualisation de l'histogramme moyen (hist_t)", fontsize=14, fontweight='bold')
    plt.xlabel("Valeurs / Bins", fontsize=12)
    plt.ylabel("Fréquence / Densité (Pondérée)", fontsize=12)
