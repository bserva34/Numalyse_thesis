...
from camera_classifier import load_model, predict_batch
...
def compute_hist_from_output(frame, out, thr, num_classes, sigma_pos,camera_prediction):
    ...
    # hist[91:96] = camera_prediction["angle_probs"] * camera_weight

    # hist[96:102] = camera_prediction["level_probs"] * camera_weight

    hist[91 + camera_prediction["angle_id"]] = 1 * camera_weight
    hist[91 + 5 + camera_prediction["level_id"]] = 1 * camera_weight
...
def main():
    ...
    parser.add_argument('--thr', type=float, default=0.75, help='Detection score threshold')
    ...
    if args.batch_size:
        ...
        batch_size = 6 if device.type == "cuda" else 1
    ...
    model_camera_angle_level = load_model("../Camera_Angle_Level/model.ckpt")

    # Nombre de classes du detecteur 
    num_classes_objects = 91
    num_classes_camera = 5 + 6
    num_classes_total = (
        num_classes_objects +
        num_classes_camera
    )
    ...
    for i in tqdm(range(num_frames), desc="Histograms"):
        ...
        if len(batch_imgs) == batch_size or i == num_frames - 1:
            ...
            results = predict_batch( model_camera_angle_level, batch_frames )


            for frame_b, out, pred in zip(batch_frames, outputs, results):

                hist = compute_hist_from_output(
                    frame_b, out,
                    args.thr, num_classes_total,
                    args.sigma_pos,
                    pred
                )
